...
* Destructor\(소멸\)
  ...
  * 명시적으로 "표시"하려면 예시의 맨 아래의 소멸자 형식처럼 사용하시면 됩니다.
...
### 상속된 Class의 Up, DownCasting

* Casting이란 보통 작은 값 형식에서 큰 값 형식으로의 변환을 의미합니다.
* Class도 Casting을 통해 선언한 객체의 타입을 변환할 수 있습니다.

#### 형변환의 이유

* 부모 클래스와 자식 클래스의 데이터 구조중 데이터의 형식이 다른 경우에 부모 클래스의 데이터 형식을 사용해야 하는 경우가 생기면, 또다시 데이터를 생성하여 메모리를 낭비하는 것보다, 이미 부모 클래스에 존재하고 있는 데이터 형식을 가져다 쓰는 것이 좋을 수 있습니다.
* 이 이외에도 여러 경우에서 자식 클래스 형식을 부모 클래스의 형식으로 Casting하는 경우가 생기는데 이때 UpCasting을 사용합니다.

DownCasting은 작은값에서 큰값으로 값형식을 변환하는 것이기 때문에 아래의 예시를 통해 간단히 알 수있습니다.
